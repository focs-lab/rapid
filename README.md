...
RAPID is a lightweight framework for implementing dynamic analyses for concurrent programs, including engines for data race prediction, atomicity checking and predictive monitoring against temporal specifications.

For the sake of simplicity, RAPID does not support its own logging mechanism but can leverage instrumentation of external tools ---  [RVPredict][2] or [RoadRunner][12] --- for logging program executions (_traces_).
...
RAPID supports four trace formats: STD, RoadRunner, RVPredict and CSV. Traces can be read in all four formats, and can be converted from any of these formats to either CSV or STD format. CSV format is close to being obsolete and STD serves as a better version than CSV.
...
The 5th event releases lock `L34` in thread `T2`.
...
Thanks to the excellent features provided by RVPredict, the development of our dynamic analysis engines could be carried on without worrying about the details of instrumentation, which can be really cumbersome. Besides, this also allowed us to compare against RVPredict's performance.
...
The CSV equivalent of the above trace is given below :
```
r(V123),,
,w(V234.23[0]),
fork(T2),,
...
The current distribution includes the following engines.

#### Race detection engines
...
8. Race detection using [WCP][1] partial order (see [`engine.racedetectionengine.wcp`](src/engine/racedetectionengine/wcp))
9. Race detection for [Sync-Preserving Races][13] (see [`engine.racedetectionengine.syncpreserving`](src/engine/racedetectionengine/syncpreserving))
10. [OSR][14] algorithm for detecting Optimistic Sync(hronization)-Reversal races (see [`engine.racedetectionengine.OSR`](src/engine/racedetectionengine/OSR))
11. Race prediction based on [sync-preserving prefix reasoning][16], enhanced with [antichain-based subsumption][15] (SeqP, see [`engine.racedetectionengine.seqpreserving`](src/engine/racedetectionengine/seqpreserving))
12. Race prediction for [Granular Prefix Races][15] combining prefix and commutativity reasoning (GrainSeqP, see [`engine.racedetectionengine.grainSeqP`](src/engine/racedetectionengine/grainSeqP))

#### Atomicity checking engines

1. [Velodrome][10] algorithm for detecting atomicity violations (see [`engine.atomicity.conflictserializability.velodome`](src/engine/atomicity/conflictserializability/velodome))
2. [Aerodrome][11] algorithm for detecting atomicity violations (see [`engine.atomicity.conflictserializability.aerodrome_basic`](src/engine/atomicity/conflictserializability/aerodrome_basic) for the basic algorithm and [`engine.atomicity.conflictserializability.aerodrome`](src/engine/atomicity/conflictserializability/aerodrome/) for Aerodrome with optimizations)
3. [AtomSanitizer][18] algorithm for testing conflict serializability in `O(nk^2)` time (see [`engine.atomicity.conflictserializability.atom_sanitizer`](src/engine/atomicity/conflictserializability/atom_sanitizer), which contains both the basic and the incremental variants)

#### Predictive monitoring engines

1. [PatternTrack][17] algorithm for predictive monitoring against pattern regular languages (see [`engine.pattern.PatternTrack`](src/engine/pattern/PatternTrack))
2. [Bertoni et al.'s algorithm][20] for membership in trace closures of regular languages, used as a baseline in [PatternTrack's evaluation][17] (see [`engine.pattern.Bertoni`](src/engine/pattern/Bertoni))
3. Predictive monitoring against pattern languages using [strong (conflict-preserving) trace prefixes][16] (see [`engine.pattern.ConfPreservingPrefix`](src/engine/pattern/ConfPreservingPrefix))

The above packages also give a fair idea on how to write your favorite dynamic analysis engine in RAPID.

**Generic Utilities** - For implementing dynamic analysis engines, one might use sophisticated data structures, such as vector clocks (VC), adaptive VCs (_epoch_), and [tree clocks][19]. We have implemented popular ones in the package `util`, and these are being used in some race detection engines.

**Parsing Utilities** - The parsing functions for reading traces in the four formats RoadRunner, RVPredict, CSV, STD are implemented in `parse` package. The parsing utilities for RVPredict format have been obfuscated and have been provided only as jar files
...
[1]: https://doi.org/10.1145/3062341.3062374
[2]: https://runtimeverification.com/predict/
[3]: https://runtimeverification.com/
[4]: https://runtimeverification.com/predict/1.8.3/docs/
[5]: https://doi.org/10.1145/781498.781529
[6]: https://doi.org/10.1145/1542476.1542490
[7]: https://doi.org/10.1145/1250734.1250762
[8]: https://doi.org/10.1145/265924.265927
[9]: https://doi.org/10.1145/3276515
[10]: https://doi.org/10.1145/1375581.1375618
[11]: https://doi.org/10.1145/3373376.3378475
[12]: https://github.com/stephenfreund/RoadRunner/
[13]: https://doi.org/10.1145/3434317
[14]: https://doi.org/10.1145/3597503.3639099
[15]: https://arxiv.org/abs/2504.10813
[16]: https://doi.org/10.1007/978-3-031-65630-9_9
[17]: https://doi.org/10.1145/3632915
[18]: https://doi.org/10.1145/3808248
[19]: https://doi.org/10.1145/3503222.3507734
[20]: https://doi.org/10.1016/0890-5401(89)90051-5
