...
//		===== This is the O(T^2) updating algo in the paper =====
	...
	public boolean existsEdge(Triplet<Integer, Integer, Long> fromEvent, Triplet<Integer, Integer, Long> toEvent, VectorClock osr, ArrayList<Long>[] inThreadIdToAuxId){
		// <threadId, inThreadId>
		int fromThId = fromEvent.first;
		int toThId = toEvent.first;
		if(fromThId == toThId) return fromEvent.second <= toEvent.second;

		VectorClock successors = this.queryForEvent(fromEvent, osr, inThreadIdToAuxId);

		if (fromEvent.third == 13L && toEvent.third == 10L) {
			System.out.println("osr = " + osr);
			System.out.println("successors = " + successors);
		}

		return successors.getClockIndex(toThId) != -1 && successors.getClockIndex(toThId) <= toEvent.second;
	}
}
