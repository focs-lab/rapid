...
public class WCPView {
	...
	private HashMap<Lock, EfficientStore> view; // LOCK -> Store
	private HashMap<Lock, HashMap<Thread, EfficientNode>> stackBottomPointerForReader; //LOCK -> (READER -> pointer_in_stack)
	...
	WCPView(HashSet<Thread> tSet) {
		...
		this.view = new HashMap<Lock, EfficientStore>();
		this.stackBottomPointerForReader = new HashMap<Lock, HashMap<Thread, EfficientNode>>();		
	...
	public void checkAndAddLock(Lock l){
		if(!lockSet.contains(l)){
			...
			lockSet.add(l);			
			view.put(l, new EfficientStore(this.threadSet.size()));
			stackBottomPointerForReader.put(l, new HashMap<Thread, EfficientNode> ());
	...
	public void pushClockPair(Lock l, WCPClockPair clockPair) {
		checkAndAddLock(l);
