...
public class WCPEvent extends RaceDetectionEvent<WCPState>{
		...
		for (Thread tPrime : state.threadToIndex.keySet()){
			if(tPrime.getId() != this.getThread().getId()){
				...
				for (Variable r : this.readVarSet) {
					if( state.existsLockReadVariableThreads.containsKey(this.getLock().getName()) ){
						if( state.existsLockReadVariableThreads.get(this.getLock().getName()).containsKey(r.getName()) ){
							if( state.existsLockReadVariableThreads.get(this.getLock().getName()).get(r.getName()).contains(tPrime.getName()) ){
								VectorClock L_r_l_x_tprime = state.getVectorClock(
										state.lastReleaseLockReadVariableThread, this.getLock(), r, tPrime);
								L_r_l_x_tprime.updateWithMax(L_r_l_x_tprime, H_t, C_t);		//Body of loop at Line 7 of algorithm. This is slightly different from the presented algorithm owing to an optimization
							}
						}
					}
				}
				...
				for (Variable w : this.writeVarSet) {
					if( state.existsLockWriteVariableThreads.containsKey(this.getLock().getName()) ){
						if( state.existsLockWriteVariableThreads.get(this.getLock().getName()).containsKey(w.getName()) ){
							if(state.existsLockWriteVariableThreads.get(this.getLock().getName()).get(w.getName()).contains(tPrime.getName())){
								VectorClock L_w_l_x_tprime = state.getVectorClock(
										state.lastReleaseLockWriteVariableThread, 
										this.getLock(), w, tPrime);
								L_w_l_x_tprime.updateWithMax(L_w_l_x_tprime, H_t, C_t);		//Body of loop at Line 8 of algorithm. This is slightly different from the presented algorithm owing to an optimization	
							}
						}
					}
				}		
	...
	public boolean HandleSubRead(WCPState state, int verbosity) {
		...
		for (Lock l : this.getLockSet()) {
			if( state.existsLockWriteVariableThreads.containsKey(l.getName()) ){
				if(state.existsLockWriteVariableThreads.get(l.getName()).containsKey(this.getVariable().getName())){
					if(state.existsLockWriteVariableThreads.get(l.getName()).get(this.getVariable().getName()).contains(this.getThread().getName())){
	...
	public boolean HandleSubWrite(WCPState state, int verbosity) {
		...
		for (Lock l : this.getLockSet()) {
			...
			if(state.existsLockWriteVariableThreads.containsKey(l.getName())){
				if(state.existsLockWriteVariableThreads.get(l.getName()).containsKey(this.getVariable().getName())){
					if(state.existsLockWriteVariableThreads.get(l.getName()).get(this.getVariable().getName()).contains(this.getThread().getName())){
						...
						P_t.updateWithMax(P_t, writeClock);	//Body of loop at Line 12 of algorithm
					}
			...
			if(state.existsLockReadVariableThreads.containsKey(l.getName())){
				if(state.existsLockReadVariableThreads.get(l.getName()).containsKey(this.getVariable().getName())){
					if(state.existsLockReadVariableThreads.get(l.getName()).get(this.getVariable().getName()).contains(this.getThread().getName())){
						...
						P_t.updateWithMax(P_t, readClock);	//Body of loop at Line 12 of algorithm
					}
		...
		//Check if an earlier read of same variable is incomparable with this write event
		if (!(R_v.isLessThanOrEqual(C_t))) {
			raceDetected = true;
		}
		//Check if an earlier write of same variable is incomparable with this write event
		if (!(W_v.isLessThanOrEqual(C_t))) {
			raceDetected = true;
		}
	...
	public boolean HandleSubFork(WCPState state, int verbosity) {
		if (state.isThreadRelevant(this.getTarget())) {
			...
			H_tc.updateWithMax(C_t, H_t);	//Update the HB predecessor of the child (forked) thread
	...
	public boolean HandleSubJoin(WCPState state, int verbosity) {
		if (state.isThreadRelevant(this.getTarget())) {
			...
			H_t.updateWithMax(H_t, H_tc, C_tc);	//Update the HB predecessor of this thread 
			P_t.updateWithMax(P_t, H_tc, C_tc);	//Update the WCP predecessor of this thread
			this.printRaceInfo(state, verbosity);
		}
		return false;
	}
