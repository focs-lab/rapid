...
public class EfficientLLViewWCP<K, V> extends EfficientLinkedList<V> {
	...
	public EfficientLLViewWCP(Set<K> keySet) {
		super();
	...
	// getMaxLowerBoundNodePointer but with curried function
	private Triplet<Boolean, V, Pair<EfficientNode<V>, Integer>> getMaxLowerBoundNodePointer(
			K key, Function<V, Boolean> comparatorLTE) {
		V maxLowerBound = null;
		boolean node_found = false;
		EfficientNode<V> iter = null;
		int iterIndex = -1;
	...
	// getMaxLowerBound but with curried comparator
	public Pair<Boolean, V> getMaxLowerBound(K key, Function<V, Boolean> comparatorLE) {
		Triplet<Boolean, V, Pair<EfficientNode<V>, Integer>> iter_triplet = this
				.getMaxLowerBoundNodePointer(key, comparatorLE);
		boolean node_found = iter_triplet.first;
		V maxLowerBound = iter_triplet.second;
		EfficientNode<V> iter = iter_triplet.third.first;
		int iterIndex = iter_triplet.third.second;
